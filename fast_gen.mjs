...
async function loop(){
  ...
  while(true){
    ...
    if(!batch||batch.length===0){console.log(`FAST DONE total=${total}`); break}
    console.log(`fast batch ${batch.length} total ${total} sample ${batch[0].name}`)
    const cs=20
  ...
  console.log(`FAST FINAL withPhoto=${withPhoto} total=${total2} pending=${total2-withPhoto}`)
}
await loop()
