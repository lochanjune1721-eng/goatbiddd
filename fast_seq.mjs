...
async function loop(){
  let total=0
  while(true){
    const {data:batch}=await supa.from('people').select('id,name').is('photo_path',null).limit(40)
    if(!batch||batch.length===0){console.log(`SEQ DONE total=${total}`); break}
    console.log(`seq batch ${batch.length} total ${total} sample ${batch[0].name}`)
    for(const p of batch){
      ...
      total++
      if(total%100===0) console.log(`seq progress ${total}`)
    }
  }
  ...
  console.log(`SEQ FINAL withPhoto=${withPhoto} total=${total2} pending=${total2-withPhoto}`)
}
await loop()
