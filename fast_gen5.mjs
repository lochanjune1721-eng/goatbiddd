...
async function loop(){
  ...
  while(true){
    ...
    if(!batch||batch.length===0){console.log(`FAST5 DONE total=${total}`); break}
    console.log(`fast5 batch ${batch.length} total ${total} sample ${batch[0].name}`)
    const cs=5
    for(let i=0;i<batch.length;i+=cs){
      ...
      total+=ch.length
      if(total%200===0) console.log(`fast5 progress ${total}`)
  ...
  console.log(`FAST5 FINAL withPhoto=${withPhoto} total=${total2} pending=${total2-withPhoto}`)
}
await loop()
